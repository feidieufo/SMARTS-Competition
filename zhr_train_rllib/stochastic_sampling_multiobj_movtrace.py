...
class StochasticSampling(Exploration):
    ...
    def _get_torch_exploration_action(self, action_dist, explore, timestep):
        ...
        inputs = action_dist.inputs                     # [2,1,9]
        ...
        for (i,action) in enumerate(action_p_sup):
            ...
            valid_mo[i] = torch.where(valid!=0, valid_mo[i], valid_mo[i]-999999999)
        ...
        dist_mo = torch.distributions.categorical.Categorical(logits=valid_mo)
        ...
        if explore:
            ...
            logp_mo = dist_mo.log_prob(action)
            logp_mo = logp_mo.permute(1, 0)
            logp = torch.cat((logp, logp_mo), dim=1)

        else:
            action = dist.deterministic_sample()
            action = label[action]
            logp = torch.zeros(size=(inputs.shape[1], inputs.shape[0]*2), dtype=torch.float32)
