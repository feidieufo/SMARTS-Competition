...
def build_q_losses(policy, model, _, train_batch):
    ...
    if config["double_q"]:
        ...
        q_tp1_best_using_online_net.append(torch.argmax(q_tp1_using_online_net[0], 1))
        q_tp1_best_using_online_net.append(torch.argmin(q_tp1_using_online_net[1], 1))
        q_tp1_best_one_hot_selection = [F.one_hot(q_tp1_best_using_online,
                                                 policy.action_space.n) for q_tp1_best_using_online in q_tp1_best_using_online_net] 
        # q_tp1_best = torch.sum(q_tp1 * q_tp1_best_one_hot_selection, 1)
        q_tp1_best = [torch.sum(q * select_a, 1) for (q, select_a) in zip(q_tp1s, q_tp1_best_one_hot_selection)]
    else:
        # qtp1 = sum(q_tp1s)
        q_tp1_best = []
        q_tp1_best.append(torch.argmax(q_tp1[0], 1))
        q_tp1_best.append(torch.argmin(q_tp1[1], 1))
        q_tp1_best_one_hot_selection = [F.one_hot(
            q_tp1, policy.action_space.n) for q_tp1 in q_tp1_best]
        # q_tp1_best = torch.sum(q_tp1 * q_tp1_best_one_hot_selection, 1)
        q_tp1_best = [torch.sum(q * select_a, 1) for (q, select_a) in zip(q_tp1s, q_tp1_best_one_hot_selection)]
