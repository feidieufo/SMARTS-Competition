...
class PPOLoss:
    def __init__(self,
        ...
        for param in model._hidden_layers.parameters():
            regularization_loss += torch.norm(param)
        for param in model._logits.parameters():
            regularization_loss += torch.norm(param)        
        ...
        if use_gae:
            ...
            loss = reduce_mean_valid(
                ...
                vf_loss_coeff * vf_loss - entropy_coeff * curr_entropy + l2_coeff*regularization_loss)
        else:
...
def ppo_surrogate_loss(policy, model, dist_class, train_batch):
    ...
    policy.loss_obj = PPOLoss(
        ...
        l2_coeff=policy.config["l2_coeff"],
    )
