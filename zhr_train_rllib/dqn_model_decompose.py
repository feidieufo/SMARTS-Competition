...
class DQNTorchModel(TorchModelV2, nn.Module):
    ...
    def __init__(
        ...
        # value layer (nodes=1).
        # advantage_module.add_module("A", nn.Linear(ins, action_space.n*decompose_num))
        value_module.add_module("V", nn.Linear(ins, 1))

        self.q_branch = torch.nn.ModuleList([nn.Linear(ins, action_space.n) for i in range(decompose_num)])
    ...
    def get_advantages_or_q_values(self, model_out):
        ...
        out = self.advantage_module(model_out)
        return [q_branch(out) for q_branch in self.q_branch]
