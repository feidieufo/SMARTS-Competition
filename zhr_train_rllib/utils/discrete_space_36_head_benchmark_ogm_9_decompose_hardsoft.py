...
def reward_adapter(env_obs, env_reward):
    ...
    total_reward = np.sum([1.0 * env_reward, 0.1 * center_penalty, 1*steering_penalty])
    total_penalty = np.sum([1 * crash_penalty])

    return [total_reward/200.0, total_penalty/200.0]
