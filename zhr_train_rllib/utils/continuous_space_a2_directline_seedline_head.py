...
OBSERVATION_SPACE = gym.spaces.Dict(
    {
        ...
        "distance_each_line": gym.spaces.Box(low=-1e10, high=1e10, shape=(5,)),
        "waypoint_each_line": gym.spaces.Box(low=-1e10, high=1e10, shape=(5,)),
...
def get_distance_from_center(env_obs):
    ...
    max_len = np.max([len(path) for path in wp_paths])
    for i, wp_p in enumerate(wp_paths):
        if max_len > len(wp_p) + 20:
            closest_wps.pop(i)
...
def observation_adapter(env_obs):
    ...
    for i, closest in enumerate(closest_wps):
        ...
        lane_hwidth = closest.lane_width * 0.5
    ...
    return {
        ...
        "distance_each_line": distance_each_line,
        "waypoint_each_line": waypoint_each_line,
...
def reward_adapter(env_obs, env_reward):
    ...
    sample_wp_path = [wp_paths[max_len_lane_index][i] for i in indices]
    heading_errors = [
        abs(math.sin(wp.relative_heading(ego_state.heading))) for wp in sample_wp_path
    ]
    ...
    total_penalty = np.sum([0.1 * center_penalty, 1 * crash_penalty,
         -0.1*heading_errors[0], -0.1*0.9*heading_errors[1], -0.1*0.9*0.9*heading_errors[2]])
