...
def build_q_losses(policy, model, _, train_batch):
    ...
    if config["double_q"]:
        ...
        q_tp1_best_using_online_net = [torch.argmax(q, 1) for q in q_tp1_using_online_net]
        q_tp1_best_one_hot_selection = [F.one_hot(q,
                                                 policy.action_space.n) for q in q_tp1_best_using_online_net]
