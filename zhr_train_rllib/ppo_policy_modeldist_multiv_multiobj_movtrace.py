...
def compute_advantages(rollout,
    ...
    if use_gae:
        ...
        b_logp, pi_logp = np.split(traj[SampleBatch.ACTION_LOGP], 2, axis=-1)   # [batch, 2s]
        rho = np.exp(-b_logp + pi_logp)
        ...
        # traj[Postprocessing.VALUE_TARGETS] = (rho*gaelam + vpred_t[:-1]).copy().astype(np.float32)
        # traj[Postprocessing.ADVANTAGES] = (gaelam - gaelam.mean(0)) / (gaelam.std(0) + 1e-8)
