...
class PPOLoss:
    def __init__(self,
        ...
        clip_adv = torch.where(advantages>0, torch.where(logp_ratio >= 1+clip_param, 
                                                        (1 + clip_param)*(1+alpha) - alpha*logp_ratio,
                                                        logp_ratio) * advantages,
                                             torch.where(logp_ratio <= 1-clip_param,
                                                        (1 - clip_param)*(1+alpha) - alpha*logp_ratio,
                                                        logp_ratio) * advantages) 
        surrogate_loss = torch.min(advantages * logp_ratio, clip_adv)         
