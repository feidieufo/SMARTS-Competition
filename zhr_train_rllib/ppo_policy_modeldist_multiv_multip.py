...
from .fc_model import FCN_MultiV, FCN_MultiV_MultiObj
...
class PPOLoss:
    def __init__(self,
        ...
        self.action_dist_prob = curr_action_dist.dist.probs.mean(1)
        ...
        if use_gae:
            ...
            loss = reduce_mean_valid(
                -surrogate_loss.mean(-1) + cur_kl_coeff * action_kl.mean(0) +
                vf_loss_coeff * vf_loss.mean(-1) - entropy_coeff * curr_entropy.mean(0))
        else:
            self.mean_vf_loss = 0.0
            loss = reduce_mean_valid(-surrogate_loss.mean(-1) +
                                     cur_kl_coeff * action_kl.mean(0) -
                                     entropy_coeff * curr_entropy.mean(0))
        self.loss = loss
