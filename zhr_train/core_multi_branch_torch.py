...
class Actor(torch.nn.Module):
    ...
    def select_action(self, s, c):
        with torch.no_grad():
            mean, std = self(s)
            c = c.unsqueeze(1)
            cc = torch.cat([c*2, c*2+1], dim=-1)
            mean_c = torch.gather(mean, dim=-1, index=cc)

            normal = Normal(mean_c, std)
            action = normal.sample()                  # [1, a_dim]
            action = torch.squeeze(action, dim=0)     # [a_dim]

        return action

    def log_pi(self, s, a, c):
        mean, std = self(s)
        c = c.unsqueeze(1)
        cc = torch.cat([c*2, c*2+1], dim=-1)
        mean_c = torch.gather(mean, dim=-1, index=cc)
...
class PPO(torch.nn.Module):
    ...
    def train_ac(self, s, a, adv, vs, oldv, c, is_clip_v=True):
        self.opti.zero_grad()
        logpi, entropy = self.actor.log_pi(s, a, c)
        old_logpi, _ = self.old_actor.log_pi(s, a, c)
        ...
        v = self.critic(s)
        c = c.unsqueeze(1)
        v_c = torch.gather(v, dim=-1, index=c)
        v = torch.squeeze(v_c, 1)
